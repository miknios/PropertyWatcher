...
namespace PropertyWatcher
{
	...
	void WatchTab(bool DrawControls, TArray<MemberPath>& WatchedMembers, bool* WantsToSave, bool* WantsToLoad,
	              TArray<PropertyItemCategory>& CategoryItems, TreeState* State = 0);

	void Update(FString WindowName, TArray<PropertyItemCategory>& CategoryItems, TArray<MemberPath>& WatchedMembers, UWorld* World,
	            bool* IsOpen, bool* WantsToSave, bool* WantsToLoad, bool Init)
	{
		...
		*WantsToSave = false;
		...
		// TODO: temporarily commented to try having watcher as inline window instead of separate one
		...
		ColumnInfos ColInfos = {};
		{
			...
			ColInfos.Infos.Add({
				ColumnID_Metadata, "metadata", "Metadata",
				FlagNoSort | ImGuiTableColumnFlags_DefaultHide | ImGuiTableColumnFlags_WidthFixed, ImGui::CalcTextSize("(?)").x
			});
			...
			for (auto CurrentTab : Tabs)
			{
				if (ImGui::BeginTabItem(ImGui_StoA(*CurrentTab)))
				{
					...
					ImGuiTableFlags TableFlags = ImGuiTableFlags_SizingFixedFit | ImGuiTableFlags_RowBg | ImGuiTableFlags_Borders |
						ImGuiTableFlags_Resizable | ImGuiTableFlags_Reorderable | ImGuiTableFlags_Hideable | ImGuiTableFlags_ScrollY |
					...
					if (ImGui::BeginTable("table", ColInfos.Infos.Num(), TableFlags, TableSize))
					{
						...
						// Allow overscroll.
						{
							...
							float Offset = ImGui::TableGetHeaderRowHeight() + ImGui::GetTextLineHeight() * 2 + ImGui::GetStyle().
								FramePadding.y * 2;
		...
		// My changes because GetWindowContentRegionMin/GetWindowContentRegionMax functions in ImGui became depracted
		ImVec2 WindowContentRegionMin = [&]
		{
			ImGuiWindow* Window = GImGui->CurrentWindow;
			const ImVec2 Result = ImVec2(
				Window->ContentRegionRect.Min.x - Window->Pos.x,
				Window->ContentRegionRect.Min.y - Window->Pos.y);
			return Result;
		}();
		...
		ImVec2 WindowContentRegionMax = []
		{
			ImGuiWindow* Window = GImGui->CurrentWindow;
			const ImVec2 Result = ImVec2(
				Window->ContentRegionRect.Max.x - Window->Pos.x,
				Window->ContentRegionRect.Max.y - Window->Pos.y);
			return Result;
		}();

		ImRect TargetRect(WindowContentRegionMin, WindowContentRegionMax);
		// ~ My changes because GetWindowContentRegionMin/GetWindowContentRegionMax functions in ImGui became depracted
			...
			bool DoRaytrace = false;
			{
				...
				if (ImGui::BeginPopup("SetChannelsPopup"))
				{
					for (int i = 0; i < CollisionChannelsActive.Num(); i++)
					{
						...
						ImGui::Selectable(ImGui_StoA(*Name), &CollisionChannelsActive[i], ImGuiSelectableFlags_NoAutoClosePopups);
					}
				...
				if (UpdateActors)
				{
					...
					if (SearchAroundPlayer)
					{
						TArray<AActor*> ResultActors;
						{
							...
							UKismetSystemLibrary::SphereOverlapActors(World, SpherePos, ActorsSearchRadius * 100, traceObjectTypes,
							                                          seekClass, ignoreActors, ResultActors);
						}
	...
	void WatchTab(bool DrawControls, TArray<MemberPath>& WatchedMembers, bool* WantsToSave, bool* WantsToLoad,
	              TArray<PropertyItemCategory>& CategoryItems, TreeState* State)
	{
		if (DrawControls)
		{
			if (ImGui::Button("Clear All"))
				...
				if (FunctionSection.IsOpen)
				{
					...
					else
					{
						for (int SectionIndex = 0; SectionIndex < SectionHelper.GetSectionCount(); SectionIndex++)
						{
							...
							TreeNodeState NodeState = {};
							NodeState.OverrideNoTreePush = true;
							BeginSection(CurrentSectionName, NodeState, State, StackIndex,
							             SectionIndex == 0 ? ImGuiTreeNodeFlags_DefaultOpen : 0);

							if (NodeState.IsOpen)
								for (int MemberIndex = MemberStartIndex; MemberIndex < MemberEndIndex; MemberIndex++)
									DrawItemRow(State, MakeFunctionItem(Item.Ptr, Functions[MemberIndex]), CurrentMemberPath,
									            StackIndex + 1);
	...
	FAView GetColumnCellText(PropertyItem& Item, int ColumnID, TreeState* State, TInlineComponentArray<FAView>* CurrentMemberPath,
	                         int* StackIndex)
	{
		FAView Result = "";
	...
	void DrawPropertyValue(PropertyItem& Item)
	{
		...
		else if (FSoftClassProperty* SoftClassProp = CastField<FSoftClassProperty>(Item.Prop))
		{
			...
			{
				FString PropPath = Item.Prop->GetPathName();
				ImGui::PushID(StringCast<ANSICHAR>(*PropPath).Get());
		...
		else if (FMapProperty* MapProp = CastField<FMapProperty>(Item.Prop))
		{
			...
			ImGui::Text("<%s, %s> (%d)", ImGui_StoA(*MapProp->KeyProp->GetCPPType()), ImGui_StoA(*MapProp->ValueProp->GetCPPType()),
			            Helper.Num());
		}
		...
		else if (FStructProperty* StructProp = CastField<FStructProperty>(Item.Prop))
		{
			...
			else if (StructType == "FLinearColor")
			{
				...
				if (ImGui::ColorEdit4("##FLinearColor", c))
				{
	...
	bool BeginTreeNode(const char* NameID, const char* DisplayName, TreeNodeState& NodeState, TreeState& State, int StackIndex,
	                   int ExtraFlags)
	{
		...
		else
		{
			...
			if (NodeState.HasBranches)
			{
...

				{
					...
					if (State.IsForceToggleNodeActive(StackIndex) && (NodeState.ActivatedForceToggleNodeOpenClose || NodeStateChanged) && !
						NodeState.IsOpen)
					{
	...
	bool SimpleSearchParser::ApplyTests(CachedColumnText& ColumnTexts)
	{
		...
		for (auto Command : Commands)
		{
			if (Command.Type == Command_Test)
			{
				...
				else if (Tst.Mod == Mod_Equal)
					Result =
						ColStr.Len() ? FCStringAnsi::Atod(*ColStr) == FCStringAnsi::Atod(*Tst.Ident) : false;
				else if (Tst.Mod == Mod_Greater)
					Result = ColStr.Len()
				...
				else if (Tst.Mod == Mod_GreaterEqual)
					Result = ColStr.Len()
						         ? FCStringAnsi::Atod(*ColStr) >= FCStringAnsi::Atod(*Tst.Ident)
						         : false;
				else if (Tst.Mod == Mod_LessEqual)
					Result = ColStr.Len()
						         ? FCStringAnsi::Atod(*ColStr) <= FCStringAnsi::Atod(*Tst.Ident)
						         : false;
	...
	bool ImGuiAddon::InputText(const char* label, TArray<char>& str, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback,
	                           void* user_data)
	{
